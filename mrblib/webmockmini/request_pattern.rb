module WebMockMini
  class RequestPattern
    ...
    def with(options = {}, &block)
      raise ArgumentError, '#with method invoked with no arguments. Either options hash or block must be specified.' if options.empty? && !block_given?
      assign_options(options)
      @with_block = block
      self
    end
    ...
    def to_s
      string = @method_pattern.to_s.upcase.to_s.dup
      string << " #{@uri_pattern}"
      string << " with body #{@body_pattern}" if @body_pattern
      string << " with headers #{@headers_pattern}" if @headers_pattern
    ...
    def assign_options(options)
      options = WebMockMini::Util::HashKeysStringifier.stringify_keys!(options, deep: true)
      HashValidator.new(options).validate_keys('body', 'headers', 'query', 'basic_auth')
      set_basic_auth_as_headers!(options)
      @body_pattern = BodyPattern.new(options['body']) if options.key?('body')
      @headers_pattern = HeadersPattern.new(options['headers']) if options.key?('headers')
      @uri_pattern.add_query_params(options['query']) if options.key?('query')
    end
  ...
  class URIPattern
    def initialize(pattern)
      @pattern = case pattern
                 when Addressable::URI, Addressable::Template
                   ...
                   WebMockMini::Util::URI.normalize_uri(pattern)
                 end
      @query_params = nil
    end
    ...
    def add_query_params(query_params)
      @query_params = if query_params.is_a?(Hash)
                        query_params
                      elsif query_params.is_a?(WebMockMini::Matchers::HashIncludingMatcher) \
                        || query_params.is_a?(WebMockMini::Matchers::HashExcludingMatcher)
                        query_params
                      else
                        WebMockMini::Util::QueryMapper.query_to_values(query_params, notation: Config.instance.query_values_notation)
                      end
    end
  ...
  class URIRegexpPattern < URIPattern
    def matches?(uri)
      WebMockMini::Util::URI.variations_of_uri_as_strings(uri).any? { |u| u.match(@pattern) } &&
        (@query_params.nil? || @query_params == WebMockMini::Util::QueryMapper.query_to_values(uri.query, notation: Config.instance.query_values_notation))
    end
  ...
  class URIAddressablePattern < URIPattern
    def matches?(uri)
      if @query_params.nil?
        ...
        WebMockMini::Util::URI.variations_of_uri_as_strings(uri).any? { |u| @pattern.match(u) }
      else
        # WebMockMini checks the query, Addressable checks everything else
        WebMockMini::Util::URI.variations_of_uri_as_strings(uri.omit(:query)).any? { |u| @pattern.match(u) } &&
          @query_params == WebMockMini::Util::QueryMapper.query_to_values(uri.query)
      end
    end

    def add_query_params(query_params)
      @@add_query_params_warned ||= false
      unless @@add_query_params_warned
        @@add_query_params_warned = true
        warn 'WebMockMini warning: ignoring query params in RFC 6570 template and checking them with WebMockMini'
      end
      super(query_params)
    end
  ...
  class URIStringPattern < URIPattern
    ...
    def add_query_params(query_params)
      ...
      if @query_params.is_a?(Hash) || @query_params.is_a?(String)
        query_hash = (WebMockMini::Util::QueryMapper.query_to_values(@pattern.query, notation: Config.instance.query_values_notation) || {}).merge(@query_params)
        @pattern.query = WebMockMini::Util::QueryMapper.values_to_query(query_hash, notation: WebMockMini::Config.instance.query_values_notation)
        @query_params = nil
      end
    end

    def to_s
      str = WebMockMini::Util::URI.strip_default_port_from_uri_string(@pattern.to_s)
      str += " with query params #{@query_params.inspect}" if @query_params
      str
    end
  end
  ...
  class BodyPattern
    ...
    def matches?(body, content_type = '')
      ...
      elsif @pattern.is_a?(WebMockMini::Matchers::HashIncludingMatcher)
    ...
    def body_as_hash(body, content_type)
      ...
      when :json then
        WebMockMini::Util::JSON.parse(body)
      when :xml then
        Crack::XML.parse(body)
      else
        WebMockMini::Util::QueryMapper.query_to_values(body, notation: Config.instance.query_values_notation)
      end
    end

    def assert_non_multipart_body(content_type)
      if content_type =~ %r{^multipart/form-data}
        raise ArgumentError, 'WebMockMini does not support matching body for multipart/form-data requests yet :('
      end
    end
  ...
  class HeadersPattern
    def initialize(pattern)
      @pattern = WebMockMini::Util::Headers.normalize_headers(pattern) || {}
    end
    ...
    def matches?(headers)
      if empty_headers?(@pattern)
        ...
        @pattern.each do |key, value|
          return false unless headers.key?(key) && value === headers[key]
        end
        true
      end
    end
    ...
    def to_s
      WebMockMini::Util::Headers.sorted_headers_string(@pattern)
    end

    def pp_to_s
      WebMockMini::Util::Headers.pp_headers_string(@pattern)
    end
