module WebMockMini
  class StubRegistry
    ...
    def register_global_stub
      ...
      stub = ::WebMockMini::RequestStub.new(:any, /.*/).with do |request|
        responses[request.object_id] = yield(request)
      end.to_return(->(request) { responses.delete(request.object_id) })
    ...
    def remove_request_stub(stub)
      unless request_stubs.delete(stub)
        raise "Request stub \n\n #{stub} \n\n is not registered."
      end
    end
    ...
    def request_stub_for(request_signature)
      (global_stubs + request_stubs).detect do |registered_request_stub|
