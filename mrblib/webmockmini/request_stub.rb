module WebMockMini
  class RequestStub
    ...
    def to_rack(app, _options = {})
      @responses_sequences << ResponsesSequence.new([RackResponse.new(app)])
    end

    def to_raise(*exceptions)
      @responses_sequences << ResponsesSequence.new([*exceptions].flatten.map do |e|
    ...
    def times(number)
      ...
      if @responses_sequences.empty?
        raise 'Invalid WebMock stub declaration. times(N) can be declared only after response declaration.'
      end
    ...
    def self.from_request_signature(signature)
      stub = new(signature.method.to_sym, signature.uri.to_s)
